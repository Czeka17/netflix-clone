...
function MovieList({ title, movieslist,isTvSerie }: MovieListProps) {
...
const movieListItems = useMemo(() => {
  ...
  <div className="absolute top-0 left-0 w-0 h-0 border-solid border-t-[0px] border-t-transparent
    ...
    <img src="/svg/tv.svg" className={`absolute w-20 lg:w-40 h-40 m-8 opacity-50  ${classes[`animate-tv`]}`} alt="" />
    <img src="/svg/film.svg" className={`absolute w-20 lg:w-40 h-40 m-8 opacity-50  ${classes[`animate-film`]}`} alt="" />
